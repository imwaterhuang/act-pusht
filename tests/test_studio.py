...
def test_builtin_dataset_contract() -> None:
    ...
    legacy_audit = PROJECT_ROOT / "artifacts/data_audit.json"
    if legacy_audit.exists():
        expected = json.loads(legacy_audit.read_text())["training_normalization"]["agent_position"]["mean"]
    else:
        # ACT inference uses checkpoint statistics; this legacy helper describes
        # raw data when the separate legacy train-split audit is unavailable.
        tables = [pq.read_table(p, columns=["observation.state"])
                  for p in sorted((BUILTIN_DATASET_ROOT / "data").rglob("*.parquet"))]
        states = np.asarray(pa.concat_tables(tables)["observation.state"].to_pylist(), dtype=np.float32)
        expected = states.mean(axis=0)
    assert summary.position_mean == pytest.approx(expected, abs=1e-4)
